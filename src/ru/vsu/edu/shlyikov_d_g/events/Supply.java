...
public class Supply {
    ...
    public void supply(MoneyScore ms){
        ...
        process(supplierList, ms);
    }

    private void process(List<Supplier> supplierList, MoneyScore ms){
...
//        showConsignment();
    ...
    protected void remove(Scanner scanner){
        ...
        showConsignment();
        String str = scanner.nextLine();
        for (String s:Utils.regexStr(str,"\\w+")) {
            List<Integer> list = Utils.regexInt(s, "\\w+");
            for (int i:list) {
                Consignment c = consignmentList.remove(i-1);
    ...
    private void showConsignment(){
        if (!consignmentList.isEmpty()) {
            ...
            for (Consignment c : consignmentList) {
                double price = Utils.round(c.getAmount() * c.getUnit_price(), 2);
